...
- **Token 最佳化**：只送文字給模型（去除時間軸），節省約 50% output token；參考資料僅第一段送出，後續段落全速處理
...
> [!WARNING]
> 請將上下文長度 Context 設定 **12288** 再導入模型，避免上下文長度不足。

**LM Studio 載入模型建議設定：**

| 設定項目 | 建議值 | 說明 |
|---------|--------|------|
| 上下文長度（Context Length） | `12288` | 第一段（含參考資料）約 9500 token，後續段約 6500 token |
...
**參考校正時間（最佳化後）：**
- RTX 3060 12GB + Qwen3.5-9B：2.5 小時錄音約需 **20 分鐘**
...
1. 下載安裝 [Pinokio](https://pinokio.computer/)
2. 開啟 Pinokio，點左側 **Discover**
3. 在搜尋框貼上此網址：`https://github.com/vincentchiou/srt-correction`
4. 點 **Download** → 等待下載完成
5. 點 **Install** → 等待安裝套件（約 1～3 分鐘）
6. 點 **Start** → 啟動服務
7. 點 **開啟 Web UI** 按鈕即可使用
...
3. **換用 context 較大的模型**
   ...
   推薦優先使用 context ≥ 12K 的模型（建議設 12288）。
